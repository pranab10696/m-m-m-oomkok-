...
const CONFIG = {
  ...
  COOLDOWNS: {
    ...
    DUEL: 5 * 60 * 1000,
    QUEST: 12 * 60 * 60 * 1000,
    EXPLORE: 60 * 60 * 1000,       // 1 hour
    WORK: 2 * 60 * 60 * 1000,      // 2 hours
    HEAL: 30 * 60 * 1000,          // 30 minutes
    REVIVE: 60 * 60 * 1000         // 1 hour
  },
  ...
  QUESTS: [
    ...
    {
      id: "treasure_hunter",
      name: "Treasure Hunter 🗝️",
      description: "Open 3 chests",
      reward: { gold: 120, xp: 60, chests: 1 },
      target: 3,
      type: "chests"
    },
    {
      id: "daily_grind",
      name: "Daily Grind ☀️",
      description: "Claim daily rewards 3 times",
  ...
  EXPLORATION_EVENTS: [
    ...
    { 
      ...
      emoji: "👹", 
      reward: { damage: [10, 30], xp: [30, 60] },
      chance: 25,
      description: "A wild monster attacked you!"
    },
    { 
      ...
      reward: { item: true },
      chance: 15,
      description: "A mysterious stranger gave you a gift!"
    },
    { 
      name: "Nothing", 
      emoji: "🌫️", 
      reward: {},
      chance: 30,
      description: "You explored but found nothing of value"
    }
  ],

  WORK_JOBS: [
    { name: "Blacksmith", emoji: "⚒️", min: 50, max: 100, description: "Forged weapons and armor" },
    { name: "Merchant", emoji: "📦", min: 40, max: 120, description: "Traded goods in the marketplace" },
    { name: "Farmer", emoji: "🌾", min: 30, max: 80, description: "Harvested crops" },
    { name: "Miner", emoji: "⛏️", min: 60, max: 150, description: "Mined precious ores" },
    { name: "Guard", emoji: "🛡️", min: 45, max: 90, description: "Protected the city gates" }
  ]
};
...
function formatReward(reward) {
  ...
  if (reward.item) parts.push("Random Item");
  if (reward.chests) parts.push(`${reward.chests}📦`);
...
async function handleProfileCommand(message) {
  ...
  // Get equipped items
  const weapon = user.equipment?.weapon ? `${user.equipment.weapon.emoji} ${user.equipment.weapon.name}` : 'None';
  const armor = user.equipment?.armor ? `${user.equipment.armor.emoji} ${user.equipment.armor.name}` : 'None';
  const accessory = user.equipment?.accessory ? `${user.equipment.accessory.emoji} ${user.equipment.accessory.name}` : 'None';
...
async function handleBuyCommand(message, itemId) {
  ...
  await updateQuestProgress(userId, "purchases", 1);
  ...
  const embed = new EmbedBuilder()
    ...
    .setColor(0x2ecc71)
    .setThumbnail('https://media.discordapp.net/attachments/1401982659965620265/1402352723290230815/930b2a0831cd495dafcba5388b13f328.png?ex=68939a3f&is=689248bf&hm=61cfe613d3ce1e6031e6e4505c439839162443c326823b00231b29741431693d&=&format=webp&quality=lossless&width=634&height=893');
...
async function handleInventoryCommand(message) {
  ...
  if (user.equipment) {
    const equipment = [
      `⚔️ Weapon: ${user.equipment.weapon ? `${user.equipment.weapon.emoji} ${user.equipment.weapon.name}` : 'None'}`,
      `🛡️ Armor: ${user.equipment.armor ? `${user.equipment.armor.emoji} ${user.equipment.armor.name}` : 'None'}`,
      `💍 Accessory: ${user.equipment.accessory ? `${user.equipment.accessory.emoji} ${user.equipment.accessory.name}` : 'None'}`
    ].join('\n');
...
// !addgold @user amount
async function handleAddGoldCommand(message, mentionedUser, amount) {
  if (!message.member.permissions.has('Administrator')) {
    return message.reply("❌ You need administrator permissions to use this command!");
  }

  const userId = mentionedUser.id;
  await db.add(`users.${userId}.gold`, parseInt(amount));

  const embed = new EmbedBuilder()
    .setTitle(`💰 Gold Added!`)
    .setDescription(`Added ${amount} gold to ${mentionedUser.username}'s balance`)
    .setColor(0xf1c40f);

  await message.reply({ embeds: [embed] });
}
// !addatk @user amount
async function handleAddAttackCommand(message, mentionedUser, amount) {
  if (!message.member.permissions.has('Administrator')) {
    return message.reply("❌ You need administrator permissions to use this command!");
  }

  const userId = mentionedUser.id;
  await db.add(`users.${userId}.stats.attack`, parseInt(amount));

  const embed = new EmbedBuilder()
    .setTitle(`⚔️ Attack Boosted!`)
    .setDescription(`Added +${amount} ATK to ${mentionedUser.username}'s stats`)
    .setColor(0xe74c3c);

  await message.reply({ embeds: [embed] });
}
// !addmana @user amount
async function handleAddManaCommand(message, mentionedUser, amount) {
  if (!message.member.permissions.has('Administrator')) {
    return message.reply("❌ You need administrator permissions to use this command!");
  }

  const userId = mentionedUser.id;
  await db.add(`users.${userId}.stats.maxMana`, parseInt(amount));
  await db.add(`users.${userId}.stats.mana`, parseInt(amount)); // Also refill current mana

  const embed = new EmbedBuilder()
    .setTitle(`🔮 Mana Increased!`)
    .setDescription(`Added +${amount} Max Mana to ${mentionedUser.username}`)
    .setColor(0x3498db);
...
async function handleDailyCommand(message) {
  ...
  // Update quest progress
  await updateQuestProgress(userId, "dailies", 1);
...
async function handleOpenChest(message) {
  ...
  // Update quest progress
  await updateQuestProgress(userId, "chests", 1);
...
async function handleSummonBossCommand(message) {
  ...
  serverData.boss.statusEffects = []; // Initialize statusEffects array
  ...
  const embed = new EmbedBuilder()
    ...
    .setImage('https://media.discordapp.net/attachments/1355956049114435703/1401799793872867418/49bd228433dcb73b4a0df43439d33913.png?ex=6892e8cb&is=6891974b&hm=1d51a6ce491a5f4e57394dbead1938b00f2d96a63552fa0b245255e8deb6e140&=&format=webp&quality=lossless&width=1355&height=763');
...
async function handleAttackCommand(message) {
  ...
  if (user.equipment.weapon) {
    const weapon = CONFIG.SHOP_ITEMS.find(i => i.name === user.equipment.weapon.name);
    if (weapon) {
...
async function defeatBoss(message, serverData) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setImage('https://media.discordapp.net/attachments/1377111272746778626/1402309618251595826/1ac14448073b8447e947ff440fb359ed.png?ex=6893721a&is=6892209a&hm=2a379bba1ae665788fdf53539ce4966d8ed69109b046215d9112441b2df51bbb&=&format=webp&quality=lossless&width=920&height=518')
    .addFields(
...
async function handleSkillsCommand(message) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setImage('https://media.discordapp.net/attachments/1377111272746778626/1402310057273856042/dee7e7e7c8ba9b8b044f2040aa0f3a45.png?ex=68937283&is=68922103&hm=466eba626c3289f494910726d202c6a14ff23dce8f9b4d44f1340bcf18bf6eae&=&format=webp&quality=lossless&width=920&height=514')
...
async function handleUseSkillCommand(message, skillName) {
  ...
  const skill = classSkills.find(s => 
    s.name.toLowerCase() === skillName.toLowerCase().trim() || 
    s.id === skillName.toLowerCase().trim()
  );
...
async function handleTopCommand(message, type = 'gold') {
  ...
  const embed = new EmbedBuilder()
    ...
    .setImage('https://media.discordapp.net/attachments/1377111272746778626/1402312413302489170/851ce1ff39f151d8622f6850aab256e9.png?ex=689374b5&is=68922335&hm=ab6326d20028db760a5222ac69b8a62e98977accf5c3f8f00aeb93600fe4cb42&=&format=webp&quality=lossless&width=1356&height=763');
...
async function handleQuestsCommand(message) {
  const userId = message.author.id;
  const user = await db.get(`users.${userId}`);
  ...
  const embed = new EmbedBuilder()
    ...
    .setColor(0x3498db)
    .setThumbnail('https://i.imgur.com/questbook.png');

  // Active Quests
  if (user.quests?.active?.length > 0) {
    const activeQuests = user.quests.active.map(questId => {
      const quest = CONFIG.QUESTS.find(q => q.id === questId);
      if (!quest) return null;

      const progress = user.quests.progress[questId] || 0;
      const progressBar = getProgressBar(progress, quest.target, 10);

      return `**${quest.name}**\n${progressBar} ${progress}/${quest.target}\n${quest.description}\nReward: ${formatReward(quest.reward)}`;
    }).filter(q => q !== null).join('\n\n');

    embed.addFields({ name: '✨ Active Quests', value: activeQuests, inline: false });
  } else {
    embed.addFields({ name: '✨ Active Quests', value: 'No active quests. Use `!quest start [id]` to start one!', inline: false });
  }

  // Available Quests
  const availableQuests = CONFIG.QUESTS.filter(quest => 
    !user.quests?.completed?.includes(quest.id) && 
    !user.quests?.active?.includes(quest.id)
  ).map(quest => 
    `**${quest.name}** [ID: ${quest.id}]\n${quest.description}\nReward: ${formatReward(quest.reward)}`
  ).join('\n\n');

  if (availableQuests) {
    embed.addFields({ name: '📭 Available Quests', value: availableQuests, inline: false });
  } else {
    embed.addFields({ name: '📭 Available Quests', value: 'No available quests. Complete more to unlock new ones!', inline: false });
  }

  embed.setFooter({ text: "Use !quest start [id] to begin a quest, !quest complete [id] to claim rewards" });

  await message.reply({ embeds: [embed] });
}

// !quest start [id] - Start a quest
async function handleQuestStartCommand(message, questId) {
  const userId = message.author.id;
  const user = await db.get(`users.${userId}`);
  ...
  const quest = CONFIG.QUESTS.find(q => q.id === questId);
  if (!quest) {
    return message.reply("❌ Invalid quest ID! Use `!quests` to see available quests.");
  }

  if (user.quests?.active?.includes(questId)) {
    return message.reply("ℹ️ You've already started this quest!");
  }

  if (user.quests?.completed?.includes(questId)) {
    return message.reply("ℹ️ You've already completed this quest!");
  }

  // Initialize quests object if needed
  if (!user.quests) {
    await db.set(`users.${userId}.quests`, {
      active: [],
      completed: [],
      progress: {}
    });
  ...
  await db.push(`users.${userId}.quests.active`, questId);

  // Initialize progress
  await db.set(`users.${userId}.quests.progress.${questId}`, 0);

  const embed = new EmbedBuilder()
    .setTitle(`📜 Quest Started: ${quest.name}`)
    .setDescription(quest.description)
    .addFields(
      { name: '🎯 Target', value: quest.target.toString(), inline: true },
      { name: '🎁 Reward', value: formatReward(quest.reward), inline: true }
    )
    .setColor(0x2ecc71)
    .setThumbnail('https://i.imgur.com/queststart.png');

  await message.reply({ embeds: [embed] });
}

// !quest complete [id] - Complete a quest
async function handleQuestCompleteCommand(message, questId) {
  const userId = message.author.id;
  const user = await db.get(`users.${userId}`);
  ...
  const quest = CONFIG.QUESTS.find(q => q.id === questId);
  if (!quest) {
    return message.reply("❌ Invalid quest ID! Use `!quests` to see your active quests.");
  }

  if (!user.quests?.active?.includes(questId)) {
    return message.reply("ℹ️ You haven't started this quest!");
  }

  const progress = user.quests.progress[questId] || 0;
  if (progress < quest.target) {
    return message.reply(`⌛ You haven't completed this quest yet! (${progress}/${quest.target})`);
  }

  // Remove from active
  await db.pull(`users.${userId}.quests.active`, questId);

  // Add to completed
  await db.push(`users.${userId}.quests.completed`, questId);

  // Remove progress
  await db.delete(`users.${userId}.quests.progress.${questId}`);

  // Give rewards
  const rewardMessages = [];

  if (quest.reward.gold) {
    await db.add(`users.${userId}.gold`, quest.reward.gold);
    rewardMessages.push(`${quest.reward.gold}💰`);
  }

  if (quest.reward.xp) {
    await db.add(`users.${userId}.xp`, quest.reward.xp);
    rewardMessages.push(`${quest.reward.xp}🌟 XP`);
  }

  if (quest.reward.chests) {
    await db.add(`users.${userId}.chests`, quest.reward.chests);
    rewardMessages.push(`${quest.reward.chests}📦 Chests`);
  }

  if (quest.reward.item) {
    const randomItem = CONFIG.SHOP_ITEMS[Math.floor(Math.random() * CONFIG.SHOP_ITEMS.length)];
    await addItemToInventory(userId, {
      ...randomItem,
      id: 1000 + Math.floor(Math.random() * 9000) // Unique ID for reward
    });
    rewardMessages.push(`${randomItem.emoji} ${randomItem.name}`);
  }

  const embed = new EmbedBuilder()
    .setTitle(`🎉 Quest Completed: ${quest.name}`)
    .setDescription(`You've successfully completed the quest!\n\n**Rewards:**\n${rewardMessages.join('\n')}`)
    .setColor(0xf1c40f)
    .setThumbnail('https://i.imgur.com/questcomplete.png');

  await message.reply({ embeds: [embed] });
}

// !equip [id] - Equip an item
async function handleEquipCommand(message, itemId) {
  const userId = message.author.id;
  const user = await db.get(`users.${userId}`);
  ...
  if (!['weapon', 'armor', 'accessory'].includes(item.type)) {
    return message.reply("❌ This item type cannot be equipped!");
  }

  // Check if already equipped
  if (user.equipment?.[item.type]?.id == itemId) {
    return message.reply("ℹ️ This item is already equipped!");
  }

  // Unequip current if exists
  if (user.equipment?.[item.type]) {
    await addItemToInventory(userId, user.equipment[item.type]);
  }

  // Remove from inventory
  const newInventory = user.inventory
    .map(i => i.id == itemId ? {...i, amount: i.amount - 1} : i)
    .filter(i => i.amount > 0);

  await db.set(`users.${userId}.inventory`, newInventory);

  // Equip new item
  await db.set(`users.${userId}.equipment.${item.type}`, {
    id: item.id,
    name: item.name,
    type: item.type,
    emoji: item.emoji,
  ...
  const embed = new EmbedBuilder()
    .setTitle(`✅ ${item.emoji} ${item.name} Equipped!`)
    .setDescription(`You've equipped ${item.name} as your ${item.type}`)
    .setColor(CONFIG.RARITY_COLORS[item.rarity] || 0x3498db)
    .setThumbnail('https://i.imgur.com/equipitem.png');

  await message.reply({ embeds: [embed] });
}

// !unequip [type] - Unequip an item
async function handleUnequipCommand(message, type) {
  const userId = message.author.id;
  const user = await db.get(`users.${userId}`);
  ...
  // Validate type
  if (!['weapon', 'armor', 'accessory'].includes(type.toLowerCase())) {
    return message.reply("❌ Invalid equipment type! Use weapon, armor, or accessory.");
  }

  const equippedItem = user.equipment?.[type];
  if (!equippedItem) {
    return message.reply(`ℹ️ You don't have a ${type} equipped!`);
  }

  // Add back to inventory
  await addItemToInventory(userId, equippedItem);

  // Unequip - set to null
  await db.delete(`users.${userId}.equipment.${type}`);

  const embed = new EmbedBuilder()
    .setTitle(`✅ ${equippedItem.emoji} ${equippedItem.name} Unequipped!`)
    .setDescription(`You've unequipped your ${type}`)
    .setColor(0x2ecc71)
    .setThumbnail('https://i.imgur.com/unequipitem.png');

  await message.reply({ embeds: [embed] });
}

// !use [id] - Use a consumable item
async function handleUseCommand(message, itemId) {
  const userId = message.author.id;
  const user = await db.get(`users.${userId}`);
  ...
  if (item.type !== "consumable") {
    return message.reply("❌ Only consumable items can be used!");
  }
  ...
  switch(effectType) {
    case "hp":
      const newHp = Math.min(user.stats.maxHp, user.stats.hp + parseInt(value));
      await db.set(`users.${userId}.stats.hp`, newHp);
      result = `❤️ Restored ${value} HP! (${user.stats.hp} → ${newHp})`;
      break;

    case "mana":
      const newMana = Math.min(user.stats.maxMana, user.stats.mana + parseInt(value));
      await db.set(`users.${userId}.stats.mana`, newMana);
      result = `🔮 Restored ${value} Mana! (${user.stats.mana} → ${newMana})`;
      break;

    case "xpboost":
      const boostAmount = parseInt(value);
      const boostExpires = Date.now() + 60 * 60 * 1000; // 1 hour
      await db.set(`users.${userId}.xpBoost`, { amount: boostAmount, expires: boostExpires });
      result = `✨ +${boostAmount}% XP boost activated for 1 hour!`;
      break;

    default:
      return message.reply("❌ This item has no usable effect!");
  }
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(result)
    .setColor(0x2ecc71)
    .setThumbnail('https://i.imgur.com/useitem.png');

  await message.reply({ embeds: [embed] });
}

// !explore - Explore for rewards
async function handleExploreCommand(message) {
  const userId = message.author.id;
  const user = await db.get(`users.${userId}`);
  ...
  const now = Date.now();
  if (user.lastExplore && now - user.lastExplore < CONFIG.COOLDOWNS.EXPLORE) {
    const remaining = Math.ceil((CONFIG.COOLDOWNS.EXPLORE - (now - user.lastExplore)) / (60 * 1000));
    return message.reply(`⏳ You're too tired to explore! Try again in ${remaining} minutes.`);
  }

  // Get random event
  const event = getRandomEvent(CONFIG.EXPLORATION_EVENTS);

  // Apply results
  let result = `${event.emoji} **${event.name}**\n${event.description}\n\n`;
  let rewards = [];

  if (event.reward.gold) {
    const amount = getRandomInRange(event.reward.gold.join('-'));
    await db.add(`users.${userId}.gold`, amount);
    rewards.push(`${amount}💰`);
  }

  if (event.reward.xp) {
    let amount = getRandomInRange(event.reward.xp.join('-'));

    // Apply XP boost if active
    if (user.xpBoost && user.xpBoost.expires > Date.now()) {
      const boost = user.xpBoost.amount;
      amount = Math.floor(amount * (1 + boost/100));
      rewards.push(`${amount}🌟 XP (+${boost}% boost)`);
    } else {
      rewards.push(`${amount}🌟 XP`);
    }

    await db.add(`users.${userId}.xp`, amount);
  }
  ...
  if (event.reward.item) {
    ...
    await addItemToInventory(userId, {
      ...randomItem,
      id: 5000 + Math.floor(Math.random() * 5000) // Unique ID
    });
    rewards.push(`${randomItem.emoji} ${randomItem.name}`);
  }

  if (event.reward.damage) {
    const damage = getRandomInRange(event.reward.damage.join('-'));
    const newHp = Math.max(0, user.stats.hp - damage);
    await db.set(`users.${userId}.stats.hp`, newHp);
    rewards.push(`💔 Took ${damage} damage`);
  }

  result += rewards.length > 0 ? `**Rewards:**\n${rewards.join('\n')}` : "You found nothing of value.";

  await db.set(`users.${userId}.lastExplore`, now);

  const embed = new EmbedBuilder()
    .setTitle(`🌲 Exploration Results`)
    .setDescription(result)
    .setColor(0x2ecc71)
    .setThumbnail('https://i.imgur.com/exploration.png');
...
async function handleWorkCommand(message) {
  ...
  const now = Date.now();
  if (user.lastWork && now - user.lastWork < CONFIG.COOLDOWNS.WORK) {
    const remaining = Math.ceil((CONFIG.COOLDOWNS.WORK - (now - user.lastWork)) / (60 * 1000));
    return message.reply(`⏳ You're too tired to work! Try again in ${remaining} minutes.`);
  }

  const job = CONFIG.WORK_JOBS[Math.floor(Math.random() * CONFIG.WORK_JOBS.length)];
  const earnings = Math.floor(Math.random() * (job.max - job.min + 1)) + job.min;

  await db.add(`users.${userId}.gold`, earnings);
  await db.set(`users.${userId}.lastWork`, now);

  const embed = new EmbedBuilder()
    .setTitle(`💼 Worked as ${job.name} ${job.emoji}`)
    .setDescription(`You ${job.description.toLowerCase()} and earned ${earnings}💰`)
    .setColor(0xf1c40f)
    .setThumbnail('https://i.imgur.com/workjob.png');

  await message.reply({ embeds: [embed] });
}

// !duel [@user] - Challenge another player
async function handleDuelCommand(message, mentionedUser) {
  ...
  const challenger = await db.get(`users.${challengerId}`);
  const target = await db.get(`users.${targetId}`);

  if (!challenger || !target || !challenger.class || !target.class) {
    return message.reply("❌ Both players need to have created characters!");
  }

  // Check cooldowns
  const now = Date.now();
  if (challenger.lastDuel && now - challenger.lastDuel < CONFIG.COOLDOWNS.DUEL) {
    const remaining = Math.ceil((CONFIG.COOLDOWNS.DUEL - (now - challenger.lastDuel)) / (60 * 1000));
    return message.reply(`⏳ You can duel again in ${remaining} minutes.`);
  }

  if (target.lastDuel && now - target.lastDuel < CONFIG.COOLDOWNS.DUEL) {
    return message.reply(`⏳ ${mentionedUser.username} is on duel cooldown!`);
  }

  // Create duel request
  const duelId = `${challengerId}-${targetId}-${Date.now()}`;
  await db.set(`duels.${duelId}`, {
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(`${message.author.username} has challenged ${mentionedUser.username} to a duel!`)
    .addFields(
      { name: 'Challenger', value: `${message.author.username} (Level ${Math.floor(challenger.xp/100)+1})`, inline: true },
      { name: 'Target', value: `${mentionedUser.username} (Level ${Math.floor(target.xp/100)+1})`, inline: true },
      { name: 'Reward', value: 'Winner gets 100💰 and 50🌟 XP', inline: false }
    )
    .setColor(0xe74c3c)
    .setThumbnail('https://i.imgur.com/duelchallenge.png');
  ...
  await message.reply({ 
    content: `${mentionedUser} You've been challenged to a duel!`,
    embeds: [embed], 
    components: [row] 
  });
}
...
// Update quest progress
async function updateQuestProgress(userId, questType, amount) {
  const user = await db.get(`users.${userId}`);
  if (!user) return;
  ...
  const activeQuests = user.quests.active.filter(questId => {
    const quest = CONFIG.QUESTS.find(q => q.id === questId);
  ...
  for (const questId of activeQuests) {
    const currentProgress = user.quests.progress[questId] || 0;
    const newProgress = currentProgress + amount;

    await db.set(`users.${userId}.quests.progress.${questId}`, newProgress);
  }
}

// Execute a duel between two players
async function executeDuel(interaction, duelId) {
  try {
    const duel = await db.get(`duels.${duelId}`);
    if (!duel) {
      return interaction.editReply('❌ Duel expired or invalid!');
    }

    const [challenger, target] = await Promise.all([
      db.get(`users.${duel.challenger}`),
      db.get(`users.${duel.target}`)
    ]);
    ...
    const challengerAttack = challenger.stats.attack + 
      (challenger.equipment?.weapon?.effect?.includes('attack') ? 
        parseInt(challenger.equipment.weapon.effect.split(':')[1]) : 0);

    const targetAttack = target.stats.attack + 
      (target.equipment?.weapon?.effect?.includes('attack') ? 
        parseInt(target.equipment.weapon.effect.split(':')[1]) : 0);

    const challengerDefense = challenger.stats.defense + 
      (challenger.equipment?.armor?.effect?.includes('defense') ? 
        parseInt(challenger.equipment.armor.effect.split(':')[1]) : 0);

    const targetDefense = target.stats.defense + 
      (target.equipment?.armor?.effect?.includes('defense') ? 
        parseInt(target.equipment.armor.effect.split(':')[1]) : 0);

    // Battle simulation (3 rounds)
    const battleLog = [];
    let challengerHP = challenger.stats.hp;
    let targetHP = target.stats.hp;

    for (let round = 1; round <= 3; round++) {
      // Challenger attacks
      let damage = Math.max(1, 
        Math.floor(challengerAttack * (0.8 + Math.random() * 0.4)) - 
        Math.floor(targetDefense * 0.3)
      );
      targetHP = Math.max(0, targetHP - damage);
      battleLog.push(`Round ${round}: ${duel.challengerName} hits for ${damage} damage!`);

      if (targetHP <= 0) break;

      // Target counterattacks
      damage = Math.max(1, 
        Math.floor(targetAttack * (0.8 + Math.random() * 0.4)) - 
        Math.floor(challengerDefense * 0.3)
      );
      challengerHP = Math.max(0, challengerHP - damage);
      battleLog.push(`Round ${round}: ${duel.targetName} counters for ${damage} damage!`);

      if (challengerHP <= 0) break;
    }

    // Determine winner
    const winnerId = challengerHP > targetHP ? duel.challenger : duel.target;
    const loserId = challengerHP > targetHP ? duel.target : duel.challenger;

    // Update database with Promise.all for better performance
    await Promise.all([
      db.add(`users.${winnerId}.wins`, 1),
      db.add(`users.${loserId}.losses`, 1),
      db.add(`users.${winnerId}.gold`, 100),
      db.add(`users.${winnerId}.xp`, 50),
      db.set(`users.${duel.challenger}.stats.hp`, challenger.stats.maxHp),
      db.set(`users.${duel.target}.stats.hp`, target.stats.maxHp),
      db.set(`users.${duel.challenger}.lastDuel`, Date.now()),
      db.set(`users.${duel.target}.lastDuel`, Date.now()),
      db.delete(`duels.${duelId}`)
    ]);
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        { name: 'Battle Log', value: battleLog.join('\n'), inline: false },
        { name: 'Rewards', value: 'Winner received:\n• 100💰\n• 50🌟 XP', inline: false }
      )
      .setColor(0x00FF00)
      .setThumbnail('https://i.imgur.com/duelresults.png');

    await interaction.editReply({ embeds: [embed], components: [] });
    
  } catch (error) {
    console.error('Duel error:', error);
    await interaction.editReply('❌ An error occurred during the duel!');
  }
}
...
client.on('ready', () => {
  ...
  setInterval(updateLeaderboards, 30 * 60 * 1000); // Every 30 minutes
});
...
client.on('messageCreate', async (message) => {
  ...
  if (content.startsWith('!start')) {
    ...
    else if (content.startsWith('!addgold')) {
      const mentionedUser = message.mentions.users.first();
      const amount = content.split(' ')[2];
      if (!mentionedUser || !amount) {
        return message.reply("❌ Usage: `!addgold @user amount`");
      }
      await handleAddGoldCommand(message, mentionedUser, amount);
    }
    else if (content.startsWith('!addatk')) {
      const mentionedUser = message.mentions.users.first();
      const amount = content.split(' ')[2];
      if (!mentionedUser || !amount) {
        return message.reply("❌ Usage: `!addatk @user amount`");
      }
      await handleAddAttackCommand(message, mentionedUser, amount);
    }
    else if (content.startsWith('!addmana')) {
      const mentionedUser = message.mentions.users.first();
      const amount = content.split(' ')[2];
      if (!mentionedUser || !amount) {
        return message.reply("❌ Usage: `!addmana @user amount`");
      }
      await handleAddManaCommand(message, mentionedUser, amount);
    }
    ...
    const skillName = content.substring(7).trim(); // Remove '!skill ' and trim whitespace
    if (!skillName) {
      return message.reply("❌ Please specify a skill name! Use `!skills` to see available skills.");
    }
  ...
  else if (content.startsWith('!quests')) {
    await handleQuestsCommand(message);
  }
  else if (content.startsWith('!quest start')) {
    const questId = content.split(' ')[2];
    await handleQuestStartCommand(message, questId);
  }
  else if (content.startsWith('!quest complete')) {
    const questId = content.split(' ')[2];
    await handleQuestCompleteCommand(message, questId);
  }
  else if (content.startsWith('!equip')) {
    const itemId = content.split(' ')[1];
    await handleEquipCommand(message, itemId);
  }
  else if (content.startsWith('!unequip')) {
    const type = content.split(' ')[1];
    if (!type) {
      return message.reply("❌ Please specify equipment type! Usage: `!unequip [weapon/armor/accessory]`");
    }
    await handleUnequipCommand(message, type);
  }
  else if (content.startsWith('!use')) {
    const itemId = content.split(' ')[1];
    await handleUseCommand(message, itemId);
  }
  else if (content.startsWith('!explore')) {
    await handleExploreCommand(message);
  }
    ...
    const mentionedUser = message.mentions.users.first();
    if (!mentionedUser) {
      return message.reply("❌ Please mention a user to duel!");
    }
    await handleDuelCommand(message, mentionedUser);
  }
    ...
    const embed = new EmbedBuilder()
      ...
      .setImage('https://media.discordapp.net/attachments/1377111272746778626/1402312927226105936/2da23d7ce78c96f1877a8621c3318532.png?ex=6893752f&is=689223af&hm=005fcc483cee723b7e4b167d132d2d739ba7d0bb82b7c4b674b542638ea959fc&=&format=webp&quality=lossless&width=920&height=518')
      .addFields(
        { name: '🎮 Character', value: '`!start` - Begin journey\n`!profile` - View stats', inline: true },
        { name: '💰 Economy', value: '`!daily` - Daily rewards\n`!shop` - Browse shop\n`!buy [ID]` - Buy items\n`!work` - Earn gold', inline: true },
        { name: '📦 Inventory', value: '`!inventory` - View items\n`!use [ID]` - Use items\n`!equip [ID]` - Equip gear\n`!unequip [type]`', inline: true },
        { name: '⚔️ Combat', value: '`!summonboss` - Spawn boss (Mods)\n`!attack` - Attack boss\n`!skills` - View skills\n`!skill [name]`', inline: true },
        { name: '🌲 Exploration', value: '`!explore` - Discover rewards\n`!heal` - Use health potion\n`!revive` - Revive when defeated', inline: true },
        { name: '📜 Quests', value: '`!quests` - View quests\n`!quest start [id]` - Begin quest\n`!quest complete [id]` - Claim reward', inline: true },
        { name: '⚔️ PvP', value: '`!duel @user` - Challenge another player', inline: true },
    ...
    const embed = new EmbedBuilder()
      ...
      .setImage('https://media.discordapp.net/attachments/1377111272746778626/1402313381280616448/e49f52a4bd76ebd5c5b114e48d4195ea.png?ex=6893759b&is=6892241b&hm=3a335d9622ff5f7481417788f49fcdb4cad67d5c73f19bf26a13b9b362250b70&=&format=webp&quality=lossless&width=920&height=518')
      .addFields(
  ...
  // Update message count for "Social Butterfly" quest
  if (!message.author.bot) {
    await updateQuestProgress(message.author.id, 'messages', 1);
  }
});
...
client.on('interactionCreate', async (interaction) => {
  ...
  // Duel acceptance
    ...
    const duelId = interaction.customId.split('_')[2];
    
    // Defer update to modify the original message
    await interaction.deferUpdate();
    
    const duel = await db.get(`duels.${duelId}`);

    if (!duel) {
      await interaction.editReply({ content: "This duel request no longer exists!" });
      return;
    }

    if (duel.accepted) {
      await interaction.editReply({ content: "This duel has already been accepted!" });
      return;
    }

    if (interaction.user.id !== duel.target) {
      await interaction.editReply({ content: "This duel isn't for you!" });
      return;
    }

    // Check if duel is too old (expired after 5 minutes)
    const now = Date.now();
    if (now - duel.timestamp > 5 * 60 * 1000) {
      await db.delete(`duels.${duelId}`);
      await interaction.editReply({ content: "This duel request has expired! (Duels expire after 5 minutes)" });
      return;
    }

    // Mark as accepted
    await db.set(`duels.${duelId}.accepted`, true);

    // Start duel
    await executeDuel(interaction, duelId);
  }
